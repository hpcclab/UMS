...
async function checkpoint(request: FastifyRequest<{Body: CheckpointRequestType}>) {
    ...
    const responses = await Promise.all(
        containerInfos
            ...
            .concat(
                containerInfos.map(containerInfo => checkpointContainerFileSystem(checkpointId, containerInfo, request.log)),
                volumes.map(volume => checkpointVolume(checkpointId, volume, request.log))
            )
    )
...
async function checkpointContainerFileSystem(checkpointId: string, containerInfo: any, log: FastifyLoggerInstance) {
