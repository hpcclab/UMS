...
async function transfer(request: FastifyRequest<{ Body: MigrateRequestType }>) {
    ...
    responses = await Promise.all(
        containerInfos
            .map(containerInfo => transferContainerImage(checkpointId, interfaceHost, interfacePort, containers, containerInfo, request.log))
            .concat(
                containerInfos.map(containerInfo => transferContainerFileSystem(checkpointId, interfaceHost, interfacePort, containers, containerInfo, request.log)),
                volumes.map(volume => transferVolume(checkpointId, interfaceHost, interfacePort, volume, request.log))
            )
    )
...
async function transferContainerImage(checkpointId: string, interfaceHost: string, interfacePort: string, containers: any, containerInfo: any, log: FastifyLoggerInstance) {
    const {destinationId} = await findDestinationFileSystemId(containers, containerInfo)

    await execRsync(interfacePort, `/var/lib/docker/containers/${containerInfo.Id}/checkpoints/${checkpointId}`,
        `rsync@${interfaceHost}:/var/lib/docker/containers/${destinationId}/checkpoints`, log)
}
...
async function transferContainerFileSystem(checkpointId: string, interfaceHost: string, interfacePort: string, containers: any, containerInfo: any, log: FastifyLoggerInstance) {
    ...
    if (Name === 'overlay2' && destinationFs !== null) {
        await execRsync(interfacePort, `/checkpoints/${checkpointId}/containers/${containerInfo.Id}`, `rsync@${interfaceHost}:${destinationFs}`.slice(0, -5), log)
    }
}
...
async function transferVolume(checkpointId: string, interfaceHost: string, interfacePort: string, volume: any, log: FastifyLoggerInstance) {
    await execRsync(interfacePort, join(`/checkpoints/${checkpointId}/volumes`, volume), `rsync@${interfaceHost}:/mount`, log)
}
