...
async function findDestinationFileSystemId(containers: any, containerInfo: any) {
    ...
    if (destinationId === undefined) {
        throw {statusCode: 500, message: `Cannot find Id of the destination container: ${containerInfo.Names[0]}`}
    }
...
async function waitForIt(interfaceHost: string, interfacePort: string, log: FastifyLoggerInstance) {
    try {
        const {
            stdout,
            stderr
        } = await exec(`/app/wait-for-it.sh ${interfaceHost}:${interfacePort} -t 5`);
        log.debug(stdout)
        log.error(stderr)
    } catch (error: any) {
...
async function execRsync(port: string, source: string, destination: string, log: FastifyLoggerInstance) {
